...
export function PlanViewer({ plan }: { plan: SitePlan }) {
  return (
    <div className="panel plan-viewer">
      ...
      <div className="plan-meta">
        <span className="tag"><b>mood</b> {plan.brand_mood}</span>
        <span className="tag"><b>type</b> {plan.global_style.typography}</span>
        <span className="tag"><b>shader</b> {plan.global_style.background_shader}</span>
      </div>
          ...
          {plan.sections.map((s) => (
            <div key={s.id} className="card">
              ...
              <div className="card-tags">
                <span>{s.type}</span>
                <span>cam: {s.camera_preset}</span>
                <span>light: {s.lighting_preset}</span>
              </div>
            </div>
          ))}
        </div>
          ...
          {plan.objects.map((o) => (
            <div key={o.id} className="card">
              <div className="card-title">
                {o.id}
                <span className={`badge badge--${o.model_format}`}>{o.model_format}</span>
              </div>
              <div className="card-sub">{o.description}</div>
              <div className="card-tags">
                <span>{o.category}</span>
                <span>anim: {o.animation}</span>
